...
export function getSavedIdentity(): 'RLRJupiter' | 'Ramjet' | 'Ripster' | null {
  try {
    const v = localStorage.getItem(IDENTITY_KEY)
    if (v === 'RLRJupiter' || v === 'Ramjet' || v === 'Ripster') return v
  } catch (_) {}
  return null
}

export function saveIdentity(identity: 'RLRJupiter' | 'Ramjet' | 'Ripster'): void {
  try {
